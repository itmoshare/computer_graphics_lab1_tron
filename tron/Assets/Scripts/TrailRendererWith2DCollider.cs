...
public class TrailRendererWith2DCollider : MonoBehaviour
{
    ...
    private bool TryAddVertices()
    {
        ...
        if ((centerPositions.First.Value - trans.position).sqrMagnitude > vertexDistanceMin * vertexDistanceMin)
        {
            ...
            var upVert = new Vector3(trans.position.x, trans.position.y, trans.position.z);
            Vector3 leftPos = upVert + (cross * -widthStart * 0.5f);
            Vector3 rightPos = upVert + (cross * widthStart * 0.5f);

            
            //create two new vertices at the calculated positions
            leftVertices.AddFirst(new Vertex(leftPos, upVert, (leftPos - trans.position).normalized));
            rightVertices.AddFirst(new Vertex(rightPos, upVert, (rightPos - trans.position).normalized));
